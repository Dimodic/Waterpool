...
# Включаем echo=True для логирования SQL (можно отключить в проде)
...
class TrainerSchedule(Base):
    ...
    __table_args__ = (
        # убрали жёстко заданное имя — SQLAlchemy сам придумает уникальный index name
...
# --- 3. инициализация БД ------------------------------------------------------
def init_db():
    """Создаём подключение, таблицы и дефолтные записи."""
    # 3.1 Проверим подключение
    try:
    ...
    # 3.2 Попытка создать таблицы
    try:
        Base.metadata.create_all(bind=ENGINE)
    except DBAPIError as e:
        # e.orig — оригинальная ошибка драйвера (psycopg2), не редактированная Streamlit
        orig = getattr(e, "orig", e)
        st.error(f"Ошибка при создании таблиц:\n{orig}")
        st.stop()

    # 3.3 Добавление дефолтных данных
    ...
    with SessionLocal() as db:
        ...
        # Пример: можно также добавить тренеров по умолчанию здесь
        db.commit()
